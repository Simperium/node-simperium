...
var _diff_match_patch = _interopRequireWildcard(require("./diff_match_patch"));
...
jsondiff.prototype.patch_apply_with_offsets = function (patches, text, offsets) {
  ...
  jsondiff.dmp.patch_splitMax(patches); // delta keeps track of the offset between the expected and actual location
  ...
  for (var x = 0; x < patches.length; x++) {
    ...
    if (start_loc == -1) {
      ...
      */
      ...
      if (text1.length > jsondiff.dmp.Match_MaxBits && jsondiff.dmp.diff_levenshtein(diffs) / text1.length > jsondiff.dmp.Patch_DeleteThreshold) {// The end points match, but the content is unacceptably bad.
