...
internal.buildModifyChange = function (id, object, ghost) {
  var payload = _util2.change.buildChange(_util2.change.type.MODIFY, id, object, ghost),
      empty = true,
      key;
...
internal.buildRemoveChange = function (id, ghost) {
  var payload = _util2.change.buildChange(_util2.change.type.REMOVE, id, {}, ghost);
...
internal.updateObjectVersion = function (id, version, data, original, patch, acknowledged) {
  var notify, changes, change, patch, localModifications, remoteModifications, transformed, update; // If it's not an ack, it's a change initiated on a different client
  ...
  if (!acknowledged) {
    ...
    localModifications = _util2.change.compressChanges(changes, original);
    remoteModifications = patch;
    transformed = _util2.change.transform(localModifications, remoteModifications, original);
    update = data; // apply the transformed patch and emit the update

    if (transformed) {
      patch = transformed;
      update = _util2.change.apply(transformed, data); // queue up the new change

      change = _util2.change.modify(id, version, patch);
...
internal.requestObjectVersion = function (id, version) {
  ...
  return new Promise(function (resolve) {
    _this2.once("version.".concat(id, ".").concat(version), function (data) {
      resolve(data);
    });

    _this2.send("e:".concat(id, ".").concat(version));
  });
};
...
internal.applyChange = function (change, ghost) {
  ...
  var error, original, patch, modified; // attempt to apply the change
  ...
  if (change.o === operation.MODIFY) {
    if (ghost && ghost.version !== change.sv) {
      internal.requestObjectVersion.call(this, change.id, change.sv).then(function (data) {
        internal.applyChange.call(_this3, change, {
          version: change.sv,
          data: data
        });
      });
    ...
    original = ghost.data;
    patch = change.v;
    modified = _util2.change.apply(patch, original);
...
internal.handleChangeError = function (err, change, acknowledged) {
  ...
  switch (err.code) {
    ...
    case CODE_EMPTY_RESPONSE:
      // Change causes no change, just acknowledge it
...
function Channel(appid, access_token, store, name) {
  ...
  this.session_id = 'node-' + (0, _uuid.v4)(); // These are the simperium bucket commands the channel knows how to handle
  ...
  message.on('o', function () {}); // Maintain a queue of operations that come from simperium commands
  // so that the can be applied to the ghost data.

  this.networkQueue = new NetworkQueue(); // Maintain a queue of operations that originate from this client
  // to track their status.

  this.localQueue = new LocalQueue(this.store); // When a local queue has indicatie that it should send a change operation
  ...
  this.localQueue.on('send', function (data) {
    _this6.emit('send', "c:".concat(JSON.stringify(data)));
...
Channel.prototype.getRevisions = function (id) {
  ...
  return new Promise(function (resolve, reject) {
    collectionRevisions(_this7, id, function (error, revisions) {
      if (error) {
        reject(error);
        return;
      }
...
Channel.prototype.reload = function () {
  ...
  this.store.eachGhost(function (ghost) {
    _this8.emit('update', ghost.key, ghost.data);
  });
};
/**
...
Channel.prototype.onAuth = function (data) {
  ...
  try {
    ...
    this.once('ready', function () {
      _this9.localQueue.resendSentChanges();
    });

    init = function init(cv) {
      if (cv) {
        _this9.localQueue.start();

        _this9.sendChangeVersionRequest(cv);
      } else {
        _this9.startIndexing();
      }
    };
...
Channel.prototype.onConnect = function () {
  ...
  this.send((0, _util.format)('init:%s', JSON.stringify(init)));
};
...
Channel.prototype.sendIndexRequest = function (mark) {
  this.send((0, _util.format)('i:1:%s::10', mark ? mark : ''));
};

Channel.prototype.sendChangeVersionRequest = function (cv) {
  this.send((0, _util.format)('cv:%s', cv));
};
...
(0, _util.inherits)(Queue, _events.EventEmitter); // Add a function at the end of the queue
...
(0, _util.inherits)(LocalQueue, _events.EventEmitter);
...
LocalQueue.prototype.processQueue = function (id) {
  ...
  var compressAndSend = this.compressAndSend.bind(this, id); // there is no queue, don't do anything

  if (!queue) return; // queue is empty, delete it from memory
...
LocalQueue.prototype.compressAndSend = function (id, ghost) {
  ...
  var c;
  var type; // a change was sent before we could compress and send
  ...
  if (changes.length > 1 && changes[0].type === _util2.change.type.REMOVE) {
    change = changes.shift();
  ...
  while (changes.length > 0) {
    ...
    if (c.o === _util2.change.type.REMOVE) {
      changes.unshift(c);
      break;
    }

    target = _util2.change.apply(c.v, target);
  }

  type = target === null ? _util2.change.type.REMOVE : _util2.change.type.MODIFY;
  change = _util2.change.buildChange(type, id, target, ghost);
  this.sent[id] = change;
  this.emit('send', change);
};
...
function collectionRevisions(channel, id, callback) {
  ...
  function onVersion(id, version, data) {
    revisionCache.set("".concat(id, ".").concat(version), data);
    versions.push({
      id: id,
      version: version,
      data: data
    ...
    fetchNextVersion(version); // defer the final response to the application
  ...
  function finish() {
    clearTimeout(timeout);
    channel.removeListener("version.".concat(id), onVersion); // sort newest first
  ...
  function fetchNextVersion(prevVersion) {
    var version = prevVersion; // find the next version to request
    ...
    requestedVersions.add(version); // fetch from server or local cache
  ...
  channel.on("version.".concat(id), onVersion); // request the first revision and start the sequence
  // pre-emptively fetch as many as could exist by default

  channel.store.get(id).then(function (_ref) {
    var version = _ref.version;
    latestVersion = version; // grab latest change revisions
    ...
    var firstArchive = Math.round((version - 60) / 10) * 10 + 1; // 127 -> 67 -> 6 -> 60 -> 61

    for (var _i = 0; _i < 100 && firstArchive - 10 * _i > 0; _i++) {
      fetchNextVersion(firstArchive - 10 * _i);
    }
  }, callback); // and set an initial timeout for failed connections
