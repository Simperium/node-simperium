...
function Channel(appid, access_token, bucket, store){
  ...
  bucket.getRevisions = function(id, count, callback) {
      new RevisionsCollector();
  };
...
Channel.prototype.onVersion = function(data){
  ...
  this.emit('version', id, version, payload);
  this.emit('version.' + id, id, version, payload);
};
...
private.updateObjectVersion = function(id, version, data, original, patch, acknowledged){
...
function RevisionsCollector(channel, id, count, callback) {
  ...
  var versions = [];
  this.on('version.' + id, function(id, version, data) {
