...
var parseVersionMessage = simperiumUtil.parseVersionMessage;
...
function Channel(appid, access_token, bucket, store){
  ...
  bucket.update = function(id, object, options, callback) {
    ...
    return update.call(bucket, id, object, options, function(err, object) {
      if (!err && options.sync !== false) bucketEvents.emit('update', id, object);
  ...
  bucket.getRevisions = function(id, callback) {
      new RevisionsCollector(channel, id, callback);
  };
...
internal.updateChangeVersion = function(cv, change) {
...
internal.updateObjectVersion = function(id, version, data, original, patch, acknowledged){
  ...
  if (!acknowledged) {
    ...
    var changes = this.localQueue.dequeueChangesFor(id),
    ...
    if (transformed) {
      ...
      var change = change_util.modify(id, version, patch);
      this.localQueue.queue(change);
    }

    notify = arglock(this.emit, 'update', id, update, original, patch).bind(this);
  } else {
...
internal.applyChange = function(change, ghost){
  ...
  if (change.error) {
    var error = new Error("Could not apply change to " + ghost.key);
  ...
  if (change.o == operation.MODIFY) {
    ...
    var original = ghost.data;
    var patch = change.v;

    var modified = jsondiff.apply_object_diff(original, patch);
    ...
    return internal.removeObject.bind(this)(change.id, acknowledged).then(emit, log_error);
  }
...
function RevisionsCollector(channel, id, callback) {
