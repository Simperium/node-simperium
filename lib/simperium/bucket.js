 ...
 */
var callbackAsPromise = function callbackAsPromise(task) {
...
var deprecateCallback = function deprecateCallback(callback, promise) {
...
var promiseAPI = function promiseAPI(store) {
  return {
    get: function get(id) {
...
(0, _util.inherits)(Bucket, _events.EventEmitter);
/**
...
Bucket.prototype.setChannel = function (channel) {
  ...
  channel // forward the index and error events from the channel
  .on('index', this.onChannelIndex).on('error', this.onChannelError) // when the channel updates or removes data, the bucket should apply
  // the same updates
  .on('update', this.onChannelUpdate).on('indexingStateChange', this.onChannelIndexingStateChange).on('remove', this.onChannelRemove);
};
/**
...
Bucket.prototype.update = function (id, data, remoteUpdateInfo, options, callback) {
  ...
  var task = this.storeAPI.update(id, data, this.isIndexing).then(function (bucketObject) {
    _this2.emit('update', id, bucketObject.data, remoteUpdateInfo);

    _this2.channel.update(bucketObject, options.sync);
...
Bucket.prototype.touch = function (id, callback) {
  ...
  var task = this.storeAPI.get(id).then(function (object) {
    return _this3.update(object.id, object.data);
  });
  return deprecateCallback(callback, task);
};
/**
...
Bucket.prototype.remove = function (id, callback) {
  ...
  var task = this.storeAPI.remove(id).then(function (result) {
    _this4.emit('remove', id);

    _this4.channel.remove(id);
